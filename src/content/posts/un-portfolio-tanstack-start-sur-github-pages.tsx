...
export const post: Post = {
  ...
  body: (
    <>
      <p>
        TanStack Start est un framework full-stack : rendu côté serveur,
        server functions, routes API. GitHub Pages, lui, ne sait faire qu’une
        chose — servir des fichiers statiques depuis un CDN. À première vue,
        les deux ne devraient pas se rencontrer.
      </p>
      <p>
      ...
      <p>
        On active le prérendu dans <code>{`vite.config.ts`}</code>, et on
        laisse le crawler suivre les liens internes :
      </p>
      <pre>
        <code>{`tanstackStart({
prerender: {
  ...
  failOnError: true,
      ...
      <p>
        <code>{`crawlLinks`}</code> extrait les <code>{`<a href>`}</code> du
        HTML généré et enfile les URL trouvées. Comme la page d’accueil pointe
        vers le blog, et que le blog liste tous les articles, une seule racine
      ...
      <p>
        ...
        plutôt que <code>{`/blog.html`}</code>. C’est exactement ce qu’attend
        un hébergeur statique pour résoudre <code>{`/blog`}</code> sans
        réécriture.
      </p>
      <h2>
      ...
      <p>
        ...
        sur GitHub Pages est servi depuis <code>{`/mon-repo/`}</code>, pas
        depuis la racine. Il faut alors accorder le <code>{`base`}</code> de
        Vite et le <code>{`basepath`}</code> du routeur — et même comme ça,
        les rapports de bugs sur le sujet s’accumulent : assets qui repartent
        à la racine, routes internes <code>{`__tsr/*`}</code> qui ignorent le
        préfixe.
      </p>
      <p>
      ...
      <p>
        <strong><code>{`.nojekyll`}</code></strong> — sans lui, GitHub Pages
        fait passer le site par Jekyll, qui ignore silencieusement tout
        fichier ou dossier commençant par un underscore. Or Start en produit :{' '}
      ...
      <p>
        ...
        n’est pas une contrainte : le contenu est écrit en dur, il change
        quand on commit, et un formulaire de contact se remplace très bien par
      ...
      <p>
        ...
        instantanée une fois le bundle chargé. Hébergement gratuit, TLS
        inclus, aucun serveur à surveiller.
      </p>
    </>
  ),
}
